...
└─────────────────────┘      └───────┬─────┬─────────┘
          ...
          └───────┬─────────────────┘  └─────┬───────┘
                  ▼                           ▼
        EpisodicMemoryService ↔ MemoryRetrieval (Qdrant + Cohere)
```
...
* **Episodic Memory** (service + retrieval) stores/recalls prior conversations via Qdrant (vector store) and Cohere (embedding). 
...
| **MemoryRetrieval** | `memory/retrieval.py` | Embeds queries (Cohere), runs hybrid search (Qdrant), returns top episodes as context. Also indexes new episodes. | `MemoryRetrievalResult`, `MemoryRetrievalResultItem` |
...
| **Embedding Service** | Cohere (`integrations.cohere_client`) |
...
User ──► JarvisBrain
   │          │
   │          ▼
   │   MemoryRetrieval ←─ Cohere (embed) & Qdrant (search/index)
   │          │
   ▼          ▼
...
* **Alternative embedding / vector store** – Swap `CohereClient` or `QdrantClient` implementations. 
